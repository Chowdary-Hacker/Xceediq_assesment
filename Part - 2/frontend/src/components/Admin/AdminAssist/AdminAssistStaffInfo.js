...
function AdminAssistStaffInfo() {
  ...
  const addTeacher = async () => {
    if (newTeacher.trim() !== '') {
     await axios.post('http://localhost:3333/teacherInfo/reg',{newTeacher}).then(res=>{if(res.status===222){alert("successfully added.. (:")}else{alert("failed try after some time (:")}}).catch(e=>console.log(e));;
  ...
  const updateTeacher = async () => {
    if (editTeacherValue.trim() !== '') {
      ...
      await axios.put('http://localhost:3333/teacherInfo/'+editTeacherValue.email+'',{editTeacherValue}).then(res=>{if(res.status===222){alert("sucessfully edited..")}else{alert("Kindly Once check the json syntax and respective key structure.. (:")}}).catch()
      await fetchAll();
      setEditTeacherValue('');
    }
  };
  ...
  const addStudent = async () => {
    if (newStudent.trim() !== '') {
      await axios.post('http://localhost:3333/studentInfo/reg',{newStudent}).then(res=>{if(res.status===222){alert("successfully added.. (:")}else{alert("failed try after some time (:")}}).catch(e=>console.log(e));;
      setNewStudent('');
    }
  };
  ...
  const updateStudent = async () => {
    if (editStudentValue.trim() !== '') {
      await axios.put('http://localhost:3333/studentInfo/'+editStudentValue.admissionNumber+'',{editStudentValue}).then(res=>{if(res.status===222){alert("sucessfully edited..")}else{alert("Kindly Once check the json syntax and respective key structure.. (:")}}).catch()
        ...
        <div className="section">
          ...
          Enter teacher details in json/object format :
            <input
              ...
              placeholder="{email:uniqueValue, name:value, classs:{{class:value, subject:value},}, salary:value}"
            />
            <button onClick={addTeacher}>Add Teacher</button>
            <button onClick={fetchAll}>Fetch All</button> 
          </div>
          <ul>
            { Object.entries(students).map(([index, teacher]) => (
              <li key={index}>
                {editTeacherIndex === index ? (
                  <>
                    <input
                      ...
                      placeholder="{email:uniqueValue, name:value, class:{{class:value, subject:value},}, salary:value}"
        ...
        <div className="section">
          ...
          <div className="input-group">
            <input
              ...
              placeholder="{admissionNumber:value, classs:value, parentEmail:value, studentEmail:value, marks:{telugu:{assesment1:value, assessment2:value, assesment3:value, finalExam:value},hindi:{assesment1:value, assessment2:value, assesment3:value, finalExam:value},english:{assesment1:value, assessment2:value, assesment3:value, finalExam:value},maths:{assesment1:value, assessment2:value, assesment3:value, finalExam:value},science:{assesment1:value, assessment2:value, assesment3:value, finalExam:value},social:{assesment1:value, assessment2:value, assesment3:value, finalExam:value}}}"
            />
