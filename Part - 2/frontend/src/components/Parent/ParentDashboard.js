...
function ParentDashboard(){
    ...
    try{axios.post("http://localhost:3333/chat/adminParentChat",{data:messageInput, sender:'parent', reciever:'admin', parentName:fee.parentName, childAdmissionNumber:admissionNumber, date:new Date().toLocaleString()}).then(res=>{if(res.status!==222){alert("Network problem! Once check internet connection and try again!!(: ")}}).catch(error => {console.error("Error fetching messages:", error)});
