...
const adminAssistLoginModel = require("../models/adminAssistLogin");
...
router.post("/", async (req,res)=>{
    ...
    try {
        const result = await adminAssistLoginModel.findOne({ userName:userName });
        if (result) {
...
router.post("/reg",async (req,res)=>{
   let {userName,password} = req.body;
    try{
        const resultt = await adminAssistLoginModel.findOne({ userName:userName });
        if (resultt) {res.status(211).json()}
        let encr = await bcrypt.hash(password,10);
        let add = adminAssistLoginModel({userName:userName, password:encr});
       add.save();
        res.status(222).json();
        }
        catch(e){
...
router.get("/adminAssist", middleware, async (req,res)=>{
    try{
        let exist = await adminAssistLoginModel.findById(req.user.id);
