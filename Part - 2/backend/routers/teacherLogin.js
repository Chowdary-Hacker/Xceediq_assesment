...
router.post("/", async (req,res)=>{
    ...
    try {
        const result = await teacherLoginModel.findOne({ email });
        if (result) {
...
router.post("/reg",async (req,res)=>{
   let {email,password} = req.body;
    try{
        const resultt = await teacherLoginModel.findOne({ email:email });
        if (resultt) {res.status(211).json()}
        let encr = await bcrypt.hash(password,10);
        let add = teacherLoginModel({email:email, password:encr});
       add.save();
        res.status(222).json();
        }
        catch(e){
