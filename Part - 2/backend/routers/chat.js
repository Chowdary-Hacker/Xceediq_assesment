...
router.post("/studentTeacherChat",async (req,res)=>{
  let {data, sender, reciever, studentName, teacherName, teacherMail, date} = req.body;
    try{
  let student = await studentTeacherChatModel({data:data, sender:sender, reciever:reciever, studentName:studentName, teacherName:teacherName, teacherMail:teacherMail, date:date});
      student.save();
  res.status(222).json();}catch(e){res.status(200).json()}
});
